...
public:
	Material(std::shared_ptr<Texture> texture, const glm::vec3 &specular, float shininess)
	 : m_diffuse(texture->sample(0, 0)), m_specular(specular), m_shininess(shininess), m_diffuse_texture(texture) { }

	glm::vec3 diffuse(const RayIntersection &intersect) const {
		if (m_diffuse_texture) return m_diffuse_texture->sample(intersect.m_uv_coord);
		return m_diffuse;
	}
	// CGRA_END_REMOVE
};